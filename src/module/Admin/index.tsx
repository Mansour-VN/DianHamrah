...
const Admin = () => {
    ...
    return(
        <div className="flex flex-col max-h-screen h-screen">
            <div id="HeaderAdmin" className="px-4 h-10 flex items-center  bg-slate-800 border-b-2">
                <p className="p-4 text-white">نام و نشان شما: </p>
            </div>
            <div className="flex flex-row gap-2 max-w-screen">
                <div id="SidebarAdmin" className="h-screen bg-slate-800 w-1/6 flex  flex-col justify-around items-center px-2">
                    <ul className="flex gap-4 flex-col justify-center items-center w-full">
                        {Navbar.map((item)=>{
                            return (
                                <li className="w-full" key={item}>
                                  <AdminButton text={item}/>
                                </li>
                            )
                        })}
                    </ul>
